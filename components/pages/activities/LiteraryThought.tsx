...
const LiteraryThought = () => {
  ...
  const fetchArticles = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/literary-thought');
  ...
  const handleReadArticle = (articleId: number) => {
    router.push(`/activities/literary-thought/article/${articleId}`);
  };
  ...
  const transformArticle = (article: Article) => ({
    ...
    translator: article.translatorAr && article.translatorFr 
      ...
      : undefined,
    date: article.date,
    category: 'literary-thought' as const,
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="mb-8">
          ...
          <h1 className={`text-4xl font-bold mb-4 ${language === 'ar' ? 'font-cairo' : 'font-montserrat'}`}>
            {language === 'ar' ? 'أدب وفكر' : 'Littérature et pensée'}
          </h1>
          <p className="text-lg text-gray-600">
            {language === 'ar' 
              ? 'مجموعة من المقالات الأدبية والفكرية المتنوعة'
              : 'Collection d\'articles littéraires et intellectuels variés'
            }
          </p>
        </div>
...
export default LiteraryThought;