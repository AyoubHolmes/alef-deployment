...
const AdminOeilAlefProemes: React.FC = () => {
  const { language } = useLanguage();
  const [articles, setArticles] = useState<Article[]>([
    {
      id: '1',
      title: { ar: 'بروييم 1', fr: 'Proème 1' },
  ...
  const handleSubmit = () => {
    ...
    if (editingArticle) {
      ...
      toast.success(language === 'ar' ? 'تم تحديث البروييم' : 'Proème mis à jour');
    } else {
      setArticles(prev => [...prev, articleData]);
      toast.success(language === 'ar' ? 'تم إضافة البروييم' : 'Proème ajouté');
    }
  ...
  const handleDelete = (id: string) => {
    if (window.confirm(language === 'ar' ? 'هل أنت متأكد من حذف هذا البروييم؟' : 'Êtes-vous sûr de supprimer ce proème ?')) {
      setArticles(prev => prev.filter(article => article.id !== id));
      toast.success(language === 'ar' ? 'تم حذف البروييم' : 'Proème supprimé');
    }
  };
      ...
      <div dir={language === 'ar' ? 'rtl' : 'ltr'} className="p-6">
        <div className="flex justify-between items-center mb-6">
          <h1 className={`text-2xl font-bold ${language === 'ar' ? 'font-cairo' : 'font-montserrat'}`}>
            {language === 'ar' ? 'إدارة بروييم - عين الألف' : 'Gestion Proèmes - L\'œil d\'Alef'}
          </h1>
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild>
              <Button className="flex items-center gap-2" onClick={resetForm}>
                <Plus size={16} />
                {language === 'ar' ? 'إضافة بروييم جديد' : 'Nouveau Proème'}
              </Button>
            </DialogTrigger>
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>
                  {editingArticle 
                    ? (language === 'ar' ? 'تعديل البروييم' : 'Modifier le Proème')
                    : (language === 'ar' ? 'إضافة بروييم جديد' : 'Nouveau Proème')
                  }
                </DialogTitle>
              </DialogHeader>
...
export default AdminOeilAlefProemes;