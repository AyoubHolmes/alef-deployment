...
const AdminPublicationsArtChiv: React.FC = () => {
  ...
  const fetchArtChivIssues = async () => {
    try {
      const response = await fetch('/api/publication-issues?magazine=ART_CHIV');
      ...
      console.error('Error fetching ArtChiv issues:', error);
